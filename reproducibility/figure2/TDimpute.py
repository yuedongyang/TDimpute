...
### pan-cancer dataset with 33 cancers
...
rows_ind = np.loadtxt(datadir+'/RNA_DNA_combine.csv', delimiter=',',skiprows=1, usecols=0, dtype=np.str) 
    ...
    for missing_perc in [0.1,0.3,0.5,0.7,0.9]:
        for sample_count in range(1, sample_size + 1):
            shuffle_cancer = pd.read_csv(datadir+'/bootstrap_cancer_V1/' + cancertype + str(int(missing_perc * 100)) + '_' + str(
                    sample_count) + '.csv', delimiter=',', index_col=0, header=0) ##data preprocessed with log2(1+G)
			...
			normal_scale = 16.5 # maximum of RSEM value
