...
def train(drop_prob, dataset_train, dataset_test):
    ...
    with tf.variable_scope('autoencoder') as scope:
        ...
        fc_2_dropout = tf.layers.dense(inputs=fc_1_dropout, units=19027)  
...
#cancers with smaple size greater than 200
cancer_names = ['LUSC', 'KIRC', 'CESC', 'STAD', 'SARC', 'COAD','KIRP', 'LUAD', 'BLCA', 'BRCA','HNSC','LGG_','PRAD','THCA','SKCM', 'LIHC']

sample_size = 5
loss_list = np.zeros([16, 5, sample_size]) #16 is number of cancers
...
for cancertype in cancer_names:
    ...
    for missing_perc in [0.1,0.3,0.5,0.7,0.9]:
        for sample_count in range(1,sample_size+1):
			...
			# 16.5 is the maximum of RSEM value
            ...
            RNA_txt.to_csv(datadir+'/filled_data/TDimpute_self_'+cancertype+str(missing_perc*100)+'_'+str(sample_count)+'.csv')
