...
args<-commandArgs(T)
reduced_names = c("BRCA", "SARC", "LUSC", "BLCA", "KIRC", "LGG_", "PRAD", "LUAD", "LIHC", "SKCM", "HNSC", "CESC", "COAD", "KIRP", "THCA", "STAD")
datadir <- "/data"
...
for(cancertype in reduced_names){
    ...
    for(missing_perc in c(0.1,0.3,0.5,0.7,0.9)){
        for(sample_count in c(1,2,3,4,5)){
            expr_file<-paste(datadir,"/bootstrap_cancer_V1/",cancertype,missing_perc*100,"_",sample_count,".csv",sep = "") 
            print(expr_file)
        ...
        #     RDNA <- RDNA[,2:dim(RDNA)[2]] # to avoid duplicated row.names error !!! duplicate 'row.names' are not allowed
            ...
            RDNA_corrupted <- RDNA
            RDNA_corrupted[1:test_size,1:19027] <- NA
            cat('dim(RDNA_corrupted)[1]: ',dim(RDNA_corrupted)[1],'\n')
            exp <- as.data.frame(RDNA_corrupted[,1:19027])
            cpg <- as.data.frame(RDNA_corrupted[,19028:46744])
