...
RNA_DNA_combine_09_values = np.load(datadir+'/RNA_DNA_combine.npy')
comm_col = pd.read_csv(datadir+'/RNA_DNA_combine_columns.csv', header = 0, index_col = 0)
comm_ind = pd.read_csv(datadir+'/RNA_DNA_combine_index.csv', header = 0, index_col = 0)
RNA_DNA_txt = pd.DataFrame(RNA_DNA_combine_09_values, index=comm_ind.values[0:8856,:].reshape(1,8856)[0],
                        columns = comm_col.values[0:46744,:].reshape(1,46744)[0])

#smaple size greater than 200
...
for cancertype in cancer_names:
    ...
    for missing_perc in [0.1,0.3,0.5,0.7,0.9]:
        for sample_count in range(1, sample_size + 1):
            ...
            RNA_txt.to_csv(datadir+'/filled_data/TDimpute_noTF_'+cancertype+str(missing_perc*100)+'_'+str(sample_count)+'.csv')
